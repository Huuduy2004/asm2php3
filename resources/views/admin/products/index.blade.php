                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between pt-3">
                            <strong class="card-title">Quản lí sản phẩm</strong>
                            @if (session('message'))
                                <div class="alert alert-success">
                                    {{ session('message') }}
                                </div>
                            @endif
                            <a href="{{ route('createProduct') }}"><button class="btn btn-succses">Thêm</button></a>
                        </div>
                                ...
                                <tbody>
                                    @foreach ($products as $product)
                                        <tr>
                                            ...
                                            <td class="avatar">
                                                <div class="round-img">
                                                    <img class="rounded-circle" src="{{ asset('/storage/' . $product->image)  }}" alt="">
                                            ...
                                            <td>
                                                @foreach ($category as $cateName)
                                                    <span>
                                                        @if ($cateName->id == $product->category_id)
                                                            {{ $cateName->name }}
                                                        @endif
                                                    </span>
                                                @endforeach
